...
class AdaptiveVMDECGDenoiserValidator:
    ...
    def load_ptbxl_clinical_data(self):
        ...
        """
        metadata_file = "ECG-Denoising-Completed/ptbxl_data/ptbxl_database.csv"
        
        if not os.path.exists(metadata_file):
            # Fallback: Generate synthetic clinical data when real dataset not available
            print("📥 PTB-XL dataset not found, generating synthetic clinical data...")
            patient_age = np.random.randint(25, 85)
            patient_sex = np.random.randint(0, 2)
            
            # Generate clinically realistic ECG based on synthetic patient characteristics
            ...
            print(f"✅ Synthetic PTB-XL clinical data generated: {len(ecg_signal)} samples at 500 Hz")
            print(f"   Synthetic Patient: Age={patient_age}, Sex={'Male' if patient_sex else 'Female'}")
            return ecg_signal, 500, f"PTB-XL Synthetic Patient (Age={patient_age})"
